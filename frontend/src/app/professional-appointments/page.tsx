...
import { FiCalendar, FiClock, FiUser, FiInfo, FiArrowLeft, FiLogOut, FiCheckCircle, FiAlertCircle, FiXCircle } from 'react-icons/fi';
...
const ProfessionalAppointments = () => {
  ...
  const fetchAppointments = async () => {
    ...
    try {
      ...
      const filtered = allAppointments.filter(
        appt => appt.schedule.professional.id === user.id
      );
  ...
  const handleAccept = async (appointmentId: number) => {
    setLoading(true);
    try {
      await api.patch(`/appointments/${appointmentId}`, { status: 'confirmado' });
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white p-6 shadow-md">
        <div className="max-w-6xl mx-auto">
          <h1 className="text-2xl font-bold mb-2">Meus Agendamentos - Profissional</h1>
          <p className="opacity-90">Gerencie os agendamentos dos seus clientes</p>
        </div>
      </div>
            ...
            <FadeInItem delay={0.1}>
              <Card>
                ...
                <div className="space-y-3">
                  ...
                  >
                    <FiCalendar className="mr-2" /> Gerenciar Horários
                  </Button>
                      ...
                      >
                        <div className="p-4">
                          <div className="flex items-start justify-between">
                              ...
                              <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-gray-600">
                                  <div className="flex items-center">
                                    ...
                                    <span>Data: {formatarData(appt.schedule.date)}</span>
                                  </div>
                                  <div className="flex items-center">
                                  ...
                                  <div className="flex items-center">
                                    ...
                                    <span>ID do Cliente: {appt.client.id}</span>
                                  </div>
                                </div>
                              </div>
                            </div>
                            ...
                            <div>
                              {appt.status.toLowerCase() === 'pendente' && (
                                <div className="flex space-x-2">
                                  <Button
                                    onClick={() => handleAccept(appt.id)}
                                    className="bg-green-500 hover:bg-green-600 !py-1 !px-2 flex items-center"
                                  >
                                    <FiCheckCircle className="mr-1" /> Aceitar
                                  </Button>
                              ...
                              {appt.status.toLowerCase() !== 'pendente' && appt.status.toLowerCase() !== 'cancelado' && (
                                confirmCancel === appt.id ? (
                                  <div className="flex flex-col items-end space-y-2 bg-red-50 p-2 rounded-lg">
                                    <span className="text-sm text-red-600 font-medium">Confirmar cancelamento?</span>
                                    <div className="flex space-x-2">
